...
function chequearSiGano() {
  ...
  for (var fila = 0; fila < grilla.length; fila++) {
          for (var columna = 0; columna < grilla[fila].length; columna++) {
            var valorActual = grilla[fila][columna];
            if (valorActual!==grillaGanadora[fila][columna]){
              return false
            }
          }
        }
...
//funcion para elegir nivel de dificultad
function elegirNivelDificultad() {
  var dificultad = parseInt(prompt("Eliga el nivel de dificultad con el que desea jugar:\n 1.Facil \n 2.Intermedio \n 3.Díficil \n 4.Experto"));
...
alert("Felicitaciones! armaste el rompecabezas!");
     //mezcla piezas nuevamente
     elegirNivelDificultad();
     mezclarPiezas(cantMezcla);
}
